'use client'

import React, { useState, useEffect } from 'react';
import { useSpring, animated } from '@react-spring/web';
import { Wallet, Import } from 'lucide-react';
import { useAuth } from '@/contexts/AuthContext';
import CreateWalletFlow from './CreateWalletFlow';
import ImportWalletFlow from './ImportWalletFlow';
import GoogleSignIn from './GoogleSignIn';

// Custom Google icon component
const GoogleIcon = () => (
  <svg 
    viewBox="0 0 24 24" 
    width="20" 
    height="20" 
    xmlns="http://www.w3.org/2000/svg"
  >
    <path
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      fill="#4285F4"
    />
    <path
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      fill="#34A853"
    />
    <path
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      fill="#FBBC05"
    />
    <path
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      fill="#EA4335"
    />
  </svg>
);

const RegistrationModal = ({ onClose }) => {
  console.log('Rendering RegistrationModal');
  const [showOptions, setShowOptions] = useState(false);
  const [isVisible, setIsVisible] = useState(false);
  const [currentFlow, setCurrentFlow] = useState(null);
  const { setUser } = useAuth();

  useEffect(() => {
    const timer = setTimeout(() => setIsVisible(true), 50);
    return () => clearTimeout(timer);
  }, []);

  const modalSpring = useSpring({
    from: { opacity: 0, transform: 'scale(0.9)' },
    to: { opacity: isVisible ? 1 : 0, transform: isVisible ? 'scale(1)' : 'scale(0.9)' },
    config: { tension: 300, friction: 20 },
  });

  const optionsSpring = useSpring({
    opacity: showOptions ? 1 : 0,
    height: showOptions ? 'auto' : 0,
    config: { tension: 300, friction: 20 },
  });

  const handleWalletCreation = () => {
    setCurrentFlow('create-wallet');
  };

  const handleWalletCreated = async (wallet) => {
    try {
      // Update auth context with the new wallet
      setUser({
        publicKey: wallet.publicKey,
        type: wallet.type,
        address: wallet.address
      });
      
      setIsVisible(false);
      setTimeout(onClose, 300);
    } catch (error) {
      console.error('Error setting wallet:', error);
    }
  };

  const handleWalletImport = () => {
    setCurrentFlow('import-wallet');
  };

  // Render the appropriate flow or the main registration screen
  const renderContent = () => {
    switch (currentFlow) {
      case 'create-wallet':
        return (
          <CreateWalletFlow 
            onClose={() => setCurrentFlow(null)}
            onSuccess={handleWalletCreated}
          />
        );
      case 'import-wallet':
        return (
          <ImportWalletFlow
            onClose={() => setCurrentFlow(null)}
            onSuccess={handleWalletCreated}
          />
        );
      default:
        return (
          <div className="p-8">
            <h2 className="text-2xl font-bold text-center mb-6">
              Welcome to Mitsui Protocol
            </h2>
            
            {/* <button
              onClick={() => login('google')}
              className="w-full flex items-center justify-center gap-2 bg-white border-2 border-black/10 hover:border-black/20 
                       text-black py-3 px-4 rounded-xl font-medium transition-colors mb-6"
            >
              <GoogleIcon />
              Continue with Google
            </button> */}
            <GoogleSignIn />

            <div className="relative mb-6">
              <div className="absolute inset-0 flex items-center">
                <div className="w-full border-t border-black/10"></div>
              </div>
              <div className="relative flex justify-center text-sm">
                <span className="px-2 bg-white text-black/50">or</span>
              </div>
            </div>

            <button
              onClick={() => setShowOptions(!showOptions)}
              className="w-full text-black/70 hover:text-black transition-colors"
            >
              Other options
            </button>

            <animated.div style={optionsSpring} className="overflow-hidden">
              <div className="space-y-4 pt-6">
                <button
                  onClick={handleWalletCreation}
                  className="w-full flex items-center justify-center gap-2 bg-black text-white py-3 px-4 rounded-xl 
                           font-medium hover:bg-black/90 transition-colors"
                >
                  <Wallet className="w-5 h-5" />
                  Create New Wallet
                </button>

                <button
                  onClick={handleWalletImport}
                  className="w-full flex items-center justify-center gap-2 bg-black/5 text-black py-3 px-4 rounded-xl 
                           font-medium hover:bg-black/10 transition-colors"
                >
                  <Import className="w-5 h-5" />
                  Import Existing Wallet
                </button>
              </div>
            </animated.div>
          </div>
        );
    }
  };

  return (
    <animated.div 
      style={modalSpring}
      className="w-full max-w-md bg-white rounded-2xl shadow-2xl overflow-hidden"
    >
      {renderContent()}
    </animated.div>
  );
};

export default RegistrationModal; 